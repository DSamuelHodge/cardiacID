...
class AuthenticationManager: ObservableObject {
    ...
    func setupHealthStore() {
        ...
        if HKHealthStore.isHealthDataAvailable() {
            ...
            errorPublisher.send("HealthKit is not available on this device")
        }
    }
    ...
    func startMonitoring() {
        guard let healthStore = healthStore else {
            authState = .failed("HealthKit is not available")
            errorPublisher.send("HealthKit is not available")
            return
        }
        
        guard HKHealthStore.isHealthDataAvailable() else {
            authState = .failed("HealthKit is not available on this device")
            errorPublisher.send("HealthKit is not available on this device")
            return
        }
    ...
    func refreshEnrollmentStatus() {
        // Check UserDefaults for enrollment status
        let userDefaultsEnrolled = UserDefaults.standard.bool(forKey: "isEnrolled")
        
        if userDefaultsEnrolled && !isEnrolled {
            // Enrollment was completed elsewhere (e.g., on watch app)
            isEnrolled = true
            authState = .authenticated
            lastVerificationTime = Date()
