...
class SupabaseService {
    ...
    // API Configuration
    private let baseUrl = "https://bzxzugwguozsymqezvig.supabase.co"
    private var apiKey: String = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ6eHp1Z3dndW96c3ltcWV6dmlnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDg0MDk5MjcsImV4cCI6MjA2Mzk4NTkyN30.ueOrX3KTg9wAHB5QYvJ6OyjAZJVbIgRmpDaP5DCFyPQ"
    ...
    func signIn(email: String, password: String) -> AnyPublisher<User, SupabaseError> {
        ...
        // For this demo, we'll simulate credential validation
        
        print("🔐 SupabaseService: Starting sign in for \(email)")
        
        // Simulate credential validation
        let validCredentials = [
            "john.doe@acme.com": "password1234"
        ]
        
        return Future<User, SupabaseError> { promise in
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                if let validPassword = validCredentials[email], validPassword == password {
                    ...
                    print("🔐 SupabaseService: Valid credentials - User: \(String(describing: mockUser.firstName)) \(String(describing: mockUser.lastName))")
                    promise(.success(mockUser))
                } else {
                    // Invalid credentials
                    print("🔐 SupabaseService: Invalid credentials for \(email)")
        ...
        .handleEvents(
            receiveOutput: { [weak self] user in
                print("🔐 SupabaseService: User data received - \(String(describing: user.firstName)) \(String(describing: user.lastName))")
                self?.currentUser = user
                self?.isAuthenticated = true
                // In a real app, would save token to keychain
                self?.accessToken = "mock-jwt-token"
                print("🔐 SupabaseService: User state updated - isAuthenticated: \(self?.isAuthenticated ?? false)")
            },
            receiveCompletion: { completion in
                print("🔐 SupabaseService: Sign in completion - \(completion)")
            }
        )
        .eraseToAnyPublisher()
    }
    ...
    func addDevice(name: String, type: Device.DeviceType) -> AnyPublisher<Device, SupabaseError> {
